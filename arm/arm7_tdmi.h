...
public class ARM7TDMI : public Core::ThreeStageCPU {
private:
  ...
  // Helper functions for getting and setting bits in CPSR, the ARM equivalent of "flags".
  ...
  bool get_imprecise_data_abort_disable(); // Never used, GBA doesn't have an MMU.
  ...
  int get_it_length();
  // Note that this will clear the ITSTATE bits corresponding to the block state.
  void set_it_length(int len);
  bool get_it_inverted(int index);
  void set_it_inverted(int index, bool inv);
  // Technically this is 4 bits, to support SIMD greater or equal to comparisons, but the GBA doesn't have an SIMD unit, so all four bits will be the same value.
  ...
  std::shared_ptr<Core::Future> fetch(std::shared_ptr<Core::Future> bus_activity_future) override;
