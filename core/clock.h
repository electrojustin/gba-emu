...
private:
  ...
  std::shared_ptr<Future> trigger_edge(std::queue<std::function<Future()>>& listeners);
public:
  // Note that because of the locking mechanics, listeners that register new listeners will be doing so for the next edge. So for example, if a falling edge listener calls register_falling_edge_listener, that callback will be execute at the *next* falling edge, not the current one.
  void register_rising_edge_listener(std::function<std::shared_ptr<Future>()> listener);
  void register_falling_edge_listener(std::function<std::shared_ptr<Future>()> listener);
